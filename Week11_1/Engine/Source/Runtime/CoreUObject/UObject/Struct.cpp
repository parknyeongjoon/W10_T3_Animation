...
void UStruct::WriteRaw(FArchive& Ar, void* Data)
{
    if (SuperStruct)
    {
        SuperStruct->WriteRaw(Ar, Data);
    }
    for (const FProperty* Prop : Properties)
    {
        if (Prop)
        {
            ...
            Ar.WriteRaw(PropData, Prop->Size);
        }
    }
}

void UStruct::ReadRaw(FArchive& Ar, void* Data)
{
    if (SuperStruct)
    {
        SuperStruct->ReadRaw(Ar, Data);
    }
    for (const FProperty* Prop : Properties)
    {
        if (Prop)
